...
// Ported from dep-guard's scripts/classify-release-tag.mjs, generalised
// from two packages to however many --package flags are passed.
//
...
// it depends on anything the build produces. Node builtins only, for the
// same reason.
//
// Usage:
//   node scripts/classify-release-tag.mjs \
//     [--tag v1.8.0] \
//     --package @vaultcompass/vault-guard-core=1.7.0 \
//     --package @vaultcompass/vault-guard=1.7.0 \
//     --package @vaultcompass/vault-guard-mcp=1.7.0 \
//     --package @vaultcompass/vault-guard-telemetry=1.7.0 \
...
//
// VG_NPM_BIN overrides the npm executable, which is how the tests point
// the registry lookup at a stub and stay offline.
...
function main() {
  ...
  try {
    decision = classifyRelease({
      ...
      actionYmlText,
      changelogText,
      publishedVersion: makeNpmLookup(process.env.VG_NPM_BIN ?? 'npm'),
    });
  } catch (err) {
...
// made, which the workflow would read as "checks passed". Returning true
// on error surfaces the problem instead.
